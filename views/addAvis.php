...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($avis)) {
        $erreur = "Le champ Avis est obligatoire.";
    }

    // Valider la note
    $note = $_POST['note'];
    if (!is_numeric($note) || $note < 1 || $note > 5) {
        $erreur = "La note doit être un nombre entre 1 et 5.";
    }

    // Vérifier si l'ID du pack est valide
    if (is_null($idPack)) {
        $erreur = "ID du pack non fourni.";
    }
...
// Récupérez l'ID du pack depuis l'objet $avis
$idPack = $avis->getPack();

// Redirigez vers la page avisparpack.php avec l'ID du pack
header("Location: avisparpack.php?idPack=$idPack");
exit();
    }
}
?>
   ...
   Spinner End -->
    ...
    <nav class="navbar navbar-expand-lg bg-white navbar-light sticky-top p-0 wow fadeIn" data-wow-delay="0.1s">
        <a href="index.html" class="navbar-brand d-flex align-items-center px-4 px-lg-5">
            <h1 class="m-0 text-primary"><img src="img/logotext.png" alt="Doctor Image" width="190" height="50"></h1>
        </a>
...
<body>
    ...
    <form action="" method="POST">
        <table>
            <tr>
                ...
                <td>
                    <textarea id="avis" class="form-control" name="avis" rows="10" cols="50"></textarea>
    ...
    <span id="erreurNote" style="color: red"></span>
            ...
            <!-- Champ "pack" automatiquement rempli avec l'ID du pack -->
            <tr>
                <td><label for="pack">Pack :</label></td>
                <td>
                    <input type="text" id="pack" name="pack" value="<?php echo $idPack; ?>" readonly />
                    <span id="erreurPack" style="color: red"></span>
                </td>
            </tr>

            <td>
                <input type="submit" class="btn btn-primary w-100 py-3" value="Save">
            </td>
            <td>
                <input type="reset" class="btn btn-primary w-100 py-3" value="Reset">
            </td>
        </table>
